...
/// struct for typed errors of method [`post_alerts`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
...
pub async fn get_alerts(configuration: &configuration::Configuration, active: Option<bool>, silenced: Option<bool>, inhibited: Option<bool>, unprocessed: Option<bool>, filter: Option<Vec<String>>, receiver: Option<&str>) -> Result<Vec<crate::models::GettableAlert>, Error<GetAlertsError>> {
    ...
    if let Some(ref local_var_str) = unprocessed {
        local_var_req_builder = local_var_req_builder.query(&[("unprocessed", &local_var_str.to_string())]);
    }
...
/// Create new Alerts
pub async fn post_alerts(configuration: &configuration::Configuration, alerts: Vec<crate::models::PostableAlert>) -> Result<(), Error<PostAlertsError>> {
    let local_var_configuration = configuration;
    ...
    local_var_req_builder = local_var_req_builder.json(&alerts);
